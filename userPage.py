...
#create user table
c.execute('CREATE TABLE IF NOT EXISTS user (username TEXT NOT NULL PRIMARY KEY,password TEX NOT NULL,fullname TEX, contactNo TEX, HouseNo TEX, street TEX, town TEX, postcode TEX);')
my_conn.commit()
my_conn.close()

 #profile view function and gui create
def profileview():
    
    print("Button clicked to show all records")
    my_conn1 = sqlite3.connect('MainDb.db')

    data_set=my_conn1.execute("SELECT * FROM user WHERE username=?", (userId,))
 
    data= data_set.fetchall()[0]
    profile= User(data[0],data[1],data[2],data[3])
    user_Address=Address(data[4],data[5],data[6],data[7])
    
    print(user_Address.n_add_HouseNo,user_Address.n_add_postcode, flush=True)

    form2 = tk.Tk()
    form2.geometry("600x800") 
    form2.config(bg="#2c3e50")
    form2.title("My profile")
    Label(form2,text = 'User Profile ',font = ('',35),bg="#2c3e50", fg="white")  

    Label(form2,text = 'User Name: ',font = ('',15),pady=5,padx=5).grid(row=2, column=0)
    txtName = Entry(form2, textvariable=profile.n_username, font=("Calibri", 16), width=30)
    txtName.grid(row=2, column=1, padx=10, pady=10, sticky="w")

    Label(form2,text = 'Full Name:  ',font = ('',15),pady=5,padx=5).grid(row=3, column=0)
    txtfullname = Entry(form2, textvariable=profile.n_fullname, font=("Calibri", 16), width=30)
    txtfullname.grid(row=3, column=1, padx=10, pady=10, sticky="w")

    Label(form2,text = 'Contact No: ',font = ('',15),pady=5,padx=5).grid(row=4, column=0)
    txtcontactno = Entry(form2, textvariable=profile.n_contactno, font=("Calibri", 16), width=30)
    txtcontactno.grid(row=4, column=1, padx=10, pady=10, sticky="w")

    Label(form2,text = 'House No:   ',font = ('',15),pady=5,padx=5).grid(row=5, column=0)
    txthouseNo = Entry(form2, textvariable=user_Address.n_add_HouseNo, font=("Calibri", 16), width=30)
    txthouseNo.grid(row=5, column=1, padx=10, pady=10, sticky="w")

    Label(form2,text = 'Street:     ',font = ('',15),pady=5,padx=5).grid(row=6, column=0)
    txtstreet = Entry(form2, textvariable=user_Address.n_add_street, font=("Calibri", 16), width=30)
    txtstreet.grid(row=6, column=1, padx=10, pady=10, sticky="w")

    Label(form2,text = 'Town:       ',font = ('',15),pady=5,padx=5).grid(row=7, column=0)
    txttown = Entry(form2, textvariable=user_Address.n_add_town, font=("Calibri", 16), width=30)
    txttown.grid(row=7, column=1, padx=10, pady=10, sticky="w")

    Label(form2,text = 'Post Code:  ',font = ('',15),pady=5,padx=5).grid(row=8, column=0)
    txtpostcode = Entry(form2, textvariable=user_Address.n_add_postcode, font=("Calibri", 16), width=30)
    txtpostcode.grid(row=8, column=1, padx=10, pady=10, sticky="w")

    txtName.delete(0, END)
    txtName.insert(END, profile.n_username)
    txtfullname.delete(0, END)
    txtfullname.insert(END, profile.n_fullname)
    txtcontactno.delete(0, END)
    txtcontactno.insert(END, profile.n_contactno)

    txthouseNo.delete(0, END)
    txthouseNo.insert(END, user_Address.n_add_HouseNo)
    txtstreet.delete(0, END)
    txtstreet.insert(END, user_Address.n_add_street)
    txttown.delete(0, END)
    txttown.insert(END, user_Address.n_add_town)
    txtpostcode.delete(0, END)
    txtpostcode.insert(END, user_Address.n_add_postcode)
...
class main:
    ...
    def new_user(self):
        ...
        if self.n_username == "" or self.n_password == "" :           
            ...
            if c.fetchall():
                ...
                self.log()
        #Create New Account 
            insert = 'INSERT INTO user(username,password,fullname ,contactNo ,HouseNo ,street , town , postcode) VALUES(?,?,?,?,?,?,?,?)'
            ...
            ('SELECT * FROM user WHERE username = ? and password = ? and fullname = ? and contactNo = ? and HouseNo = ? and street = ? and town = ? and postcode = ?')
    ...
    #Draw Widgets
    def widgets(self):
        self.head = Label(self.master,text = 'Wellcome to CookMe !',font = ('',35),bg="#2c3e50", fg="white")     
        self.head.pack()
        ...
        title = Label(self.logf, text="Login",bg="#89A9FF", fg="white", font=("Calibri", 30, "bold"))
        title.grid(row=0,column=2, columnspan=2,  pady=20, sticky="w")

        lblName = Label(self.logf, text="View As guest", font=("Calibri", 20), bg="#89A9FF", fg="white")
        lblName.grid(row=1, column=0, padx=250, pady=10, sticky="w")
        ...
        load1 = Image.open("userhome.jpg")
        render = ImageTk.PhotoImage(load1)
        img1 = Label(self.windof, image=render)
        img1.image = render
        img1.place(x=0, y=0) 

        Label(self.windof,text = 'Home Page',font = ("Calibri", 26, "bold"),bg="#89A9FF", fg="white").grid(row=0,column=0)
        Button(self.windof, command=profileview, text='See My Profile', width=15, font=("Calibri", 16, "bold"),
                    fg="white", bg="#008000",bd=0).grid(row=1,column=1,padx=10,pady=20)
        Button(self.windof, command=myRecipeManage, text='Manage my recipe', width=15, font=("Calibri", 16, "bold"),
                    fg="white", bg="#008000",bd=0).grid(row=2,column=1,padx=10,pady=20)
        Button(self.windof, command=viewRecipe, text='View all recipes', width=15, font=("Calibri", 16, "bold"),
                    fg="white", bg="#008000",bd=0).grid(row=3,column=1,padx=10,pady=20)
        Label(self.windof,text = 'Enjoy Cooking with CookMe ! ',font = ("Calibri", 15, "bold"),bg="#89A9FF", fg="white").grid(row=4,column=1,padx=50,pady=80)
